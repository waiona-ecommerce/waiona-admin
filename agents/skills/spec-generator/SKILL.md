---
name: spec-generator
description: Primer paso del flujo SDD de Waiona Admin. Investiga el código existente y el contrato del backend, entrevista al usuario si falta información, y escribe agents/specs/NN_nombre.md con los requisitos en notación EARS. Usar antes de implementar cualquier feature, cambio o bugfix no trivial, o cuando el usuario invoca /spec-generator.
---
...
5. **Determiná el número**: mirá `agents/specs/INDEX.md` para el próximo `NN` disponible.
6. **Escribí `agents/specs/NN_nombre.md`** con esta estructura:
...
- No tocás código fuente (`app/`, `actions/`, `services/`, `core/`, `proxy.ts`) — solo `agents/specs/`.
